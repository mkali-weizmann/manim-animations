...
class Potential(ZoomedScene):
    def construct(self):
        ...
        # Create integrand and integral representations
        ...
        integral_algebraic_label = Tex(r"$\frac{ke^{ikr_{01,\max}}}{4\pi ir_{01,\max}}\intop_{S}U\left(\boldsymbol{r}_{0}\right)e^{-ik\frac{r_{01,\max}-R}{2R\cdot r_{01,\max}}s^{2}}dS$")
        integral_algebraic_label.next_to(box_integral, UP, buff=0.1).to_edge(RIGHT)
        integral_algebraic_expression_as_convolution = Tex(r"$\frac{ke^{ikr_{01,\min}}}{4\pi ir_{01,\min}}\cdot\left[e^{-ik\frac{r_{01,\min}-R}{2R\cdot r_{01,\min}}s^{2}}\otimes U\left(\boldsymbol{p}_{0}\right)\right]$")
        ...
        # Put the zoomed window somewhere (top-right here)
        self.zoomed_display.move_to(p_1_reflection) #  to_corner(UR).shift(0.2 * DOWN + 0.2 * LEFT)
